...
static void SBGDataProcess1(void)
{
	...
	mblock1.ptrRegs[SBG_START_ADR] = (uint16_t)(stamp / 1000000);

	val = *((float *)(tsk_ptr1 + 10));
	val = val * 1800.0 / 3.14;
	mblock1.ptrRegs[SBG_START_ADR + 1] = (int16_t)(val); //ROLL angle
	val = *((float *)(tsk_ptr1 + 14));
	val = val * 1800.0 / 3.14;
	mblock1.ptrRegs[SBG_START_ADR + 2] = (int16_t)(val); //PITCH angle
	val = *((float *)(tsk_ptr1 + 18));
	val = val * 1800.0 / 3.14;
	mblock1.ptrRegs[SBG_START_ADR + 3] = (int16_t)(val); //YAW angle
}
...
static void SBGDataProcess2(void)
{
	...
	val = val * 1000.0;
	mblock1.ptrRegs[SBG_START_ADR + 4] = (int16_t)(val); // X axis acc
	val = *((float *)(tsk_ptr2 + 16));
	val = val * 1000.0;
	mblock1.ptrRegs[SBG_START_ADR + 5] = (int16_t)(val); // Y axis acc
	val = *((float *)(tsk_ptr2 + 20));
	val = val * 1000.0;
	mblock1.ptrRegs[SBG_START_ADR + 6] = (int16_t)(val); // Z axis acc

	val = *((float *)(tsk_ptr2 + 24));
	val = val * 1800.0 / 3.14;
	mblock1.ptrRegs[SBG_START_ADR + 7] = (int16_t)(val); // GYRD X
	val = *((float *)(tsk_ptr2 + 28));
	val = val * 1800.0 / 3.14;
	mblock1.ptrRegs[SBG_START_ADR + 8] = (int16_t)(val); // GYRD Y
	val = *((float *)(tsk_ptr2 + 32));
	val = val * 1800.0 / 3.14;
	mblock1.ptrRegs[SBG_START_ADR + 9] = (int16_t)(val); // GYRD Z

	val = *((float *)(tsk_ptr2 + 36));
	val = val * 10.0;
	mblock1.ptrRegs[SBG_START_ADR + 10] = (uint16_t)(val); // SBG inner tempature
}
